...
public class DeleteSample {
    public DeleteSample(){
        ...
        //sql = DELETE FROM user_login WHERE id = 13
        ...
        //sql = SELECT * FROM user_login WHERE id = 14
        sql = "SELECT * FROM user_login WHERE id = 14";
        ...
        if (result instanceof ArrayList) {
            ...
            if(list.size() > 0) {
                ...
                //sql = DELETE FROM user_login WHERE id = 1231
                result = sqlService.deleteOne(map);
                System.out.println("6# result = " + result);
            }
        }

        //sql = SELECT * FROM user_login WHERE id < 1242
        sql = "SELECT * FROM user_login WHERE id < 1242";
        result = sqlService.select(sql, UserLoginMap.class);
        if (result instanceof ArrayList) {
            list = (ArrayList<TableMap>)result;
